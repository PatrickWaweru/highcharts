  ...
  return new Promise(function (resolve, reject) {
    ...
    function sortMarkdownFileContent(mdContent) {
      ...
      marked.lexer(mdContent).forEach((token, index) => {
        ...
        if (
          token.type === "heading" &&
          (token.text === "Upgrade notes" || token.text === "Bug fixes")
        ) {
          write = token.text;
          return;
        }
    ...
    function featureHTMLStructure() {
      if (changelog.features) {
        ...
        return `<h3 class="release-header">
                        <a id="${id}"></a>
                        <span>${changelog.header.productName} v${version} ${
      ...
      if (changelog.upgradeNotes.length > 0) {
        return `<div id="${changelog.header.offset}heading-${
          changelog.header.version
                    ...
                    <h4 class="card-title">
                    <button aria-label="Upgrade Notes ${id} v${version}" aria-expanded="false" class="btn btn-link collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#upgrade-notes-${
          changelog.header.offset
        }${changelog.header.version}"><span> Upgrade notes </span></button>
                    </h4>
                    </div>
                    <div id="upgrade-notes-${changelog.header.offset}${
          changelog.header.version
    ...
    function bugFixesHTMLStructure() {
      ...
      if (changelog.bugFixes.length > 0) {
        return `<div id="${changelog.header.offset}heading-${
          changelog.header.version
        }-bug-fixes"class="card-header">
                    <h4 class="card-title" aria-label="Bug Fixes ${id} v${version}">
                    <button aria-label="Bug Fixes ${id} v${version}"aria-expanded="false" class="btn btn-link collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#bug-fixes-${
          changelog.header.offset
        }${changelog.header.version}"><span> Bug fixes </span></button>
                    </h4>
                    </div>
                    <div id="bug-fixes-${changelog.header.offset}${
          changelog.header.version
        }" class="collapse" aria-labelledby="${
          changelog.header.offset
        }heading-${
